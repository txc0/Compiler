    ...
    while (right <= len && left <= right) {
        ...
        if (isDelimiter(input[right]) && left == right) {
            if (isOperator(input[right])) {
                std::cout << " Operator, Value: " << input[right] << std::endl;
            }
            right++;
            left = right;
            ...
            if (isKeyword(subStr)) {
                std::cout << " Keyword, Value: " << subStr << std::endl;
            } else if (isInteger(subStr)) {
                std::cout << " Integer, Value: " << subStr << std::endl;
            } else if (isValidIdentifier(subStr) &&
                        !isDelimiter(input[right - 1])) {
                std::cout << " Identifier, Value: " << subStr << std::endl;
            } else if (!isValidIdentifier(subStr) &&
                        !isDelimiter(input[right - 1])) {
                std::cout << " Unidentified, Value: " << subStr << std::endl;
            }
            delete[] subStr;
            left = right;
        }
